...
>(({ className, children, ...props }, ref) => (
    <AccordionPrimitive.Header className="flex">
        <AccordionPrimitive.Trigger
            ref={ref}
            className={cn(
                "flex flex-1 items-center justify-between py-4 text-2xl text-muted-foreground transition-all hover:text-foreground text-left [&[data-state=open]>svg]:rotate-[135deg]",
                className
            )}
            {...props}
        >
